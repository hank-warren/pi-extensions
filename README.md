...
| [`@hank-warren/pi-plan-mode`](packages/pi-plan-mode) | Plan mode for research and design, with a durable plan file that survives compaction, reviewed `update_plan` revisions of an existing plan, and a plan-craft doc injected by path while the mode is active |
| [`@hank-warren/pi-tasks`](packages/pi-tasks) | Phased task tracking driven by `get_tasks` and `update_tasks`: stable phase/task ids, five execution states, atomic change batches, a durable annotated Markdown document with immutable per-revision snapshots and cross-process locking, structural revisions that render a review card the user accepts or sends back, and explicit recovery when the document and the session disagree |
