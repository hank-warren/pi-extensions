...
| [`@hank-warren/pi-plan-mode`](packages/pi-plan-mode) | Plan mode for research and design, with a durable plan file that survives compaction, reviewed `update_plan` revisions with optional bound task reconciliation and task-aware completion, and a plan-craft doc injected by path while the mode is active |
| [`@hank-warren/pi-tasks`](packages/pi-tasks) | Phased task tracking driven by `get_tasks` and `update_tasks`: stable phase/task ids, five execution states, atomic change batches, a durable annotated Markdown document with immutable per-revision snapshots and cross-process locking, structural revisions that render a review card the user accepts or sends back, explicit recovery when the document and the session disagree, and optional plan-bound scope/progress tracking |
...
The [`pi-plan-mode`](packages/pi-plan-mode) package started as a fork of `@narumitw/pi-plan-mode` 0.49.3 and has since diverged substantially. Plan mode is a mode of intent, not a permission system: it blocks `edit` and `write` while planning and leaves every other tool as configured, so command review stays with [`pi-auto-permissions`](packages/pi-auto-permissions). Completed plans are written to `~/.pi/agent/plans/<session-id>.md`; the model sees only a short pointer to that file, so plans survive compaction at negligible context cost and can be hand-edited. Changing a plan that already exists is a tool call, not a file edit: `update_plan` opens a revision, proposes a complete rewrite, and shows a computed diff to accept, send back, or cancel — with revision history and snapshots under `~/.pi/agent/plans/.revisions/`, and approval bound to the exact bytes the user approved. `plan_mode_complete` refuses once a plan has history, so reviewed scope cannot be replaced from model memory; a plan whose file no longer matches what was approved is reported as unverified, refuses `edit`/`write`, and refuses completion until a person revises or confirms it. Paired with [`pi-ask-user-question`](packages/pi-ask-user-question), Plan mode asks its decision questions through that tool — previews, notes, question tabs, digit hotkeys and checkbox multi-select — and hides its own weaker `plan_mode_question`; standalone installs keep the built-in tool. See [`packages/pi-plan-mode/README.md`](packages/pi-plan-mode/README.md) for details.
