...
export function registerCodexCompactionExtension(pi: ExtensionAPI, hostVersion = VERSION): void {
	...
	let cpaProviders = loadCpaProviders();
	const supportsNativeCompaction = (model: unknown): model is Model<"openai-codex-responses" | "openai-responses"> =>
		isNativeCompactionModel(model, cpaProviders);
	...
	const createNativeCheckpoint = async (params: {
		...
		const directCodex = isOpenAICodexModel(params.model);
		const headerParams = { apiKey: auth.apiKey, headers: auth.headers, sessionId };
		const remote = await callRemoteCompaction({
			url: directCodex ? resolveCodexResponsesUrl(params.model.baseUrl) : resolveCpaResponsesUrl(params.model.baseUrl),
			headers: directCodex ? buildCodexHeaders(headerParams) : buildCpaHeaders(headerParams),
			body,
			model: params.model,
	...
	pi.on("before_provider_headers", (event, ctx) => {
		if (!supportsNativeCompaction(ctx.model)) return;
		setFeatureHeader(event.headers);
	});

	pi.on("before_provider_request", async (event, ctx) => {
		const model = ctx.model;
		if (!supportsNativeCompaction(model) || !isJsonObject(event.payload)) return undefined;
		...
		try {
			...
			// Generic Responses carries system instructions in input, unlike direct
			// Codex's top-level instructions. Preserve the finalized prompt (including
			// earlier payload hooks), but replace the conversation and local marker.
			const instructions = !isOpenAICodexModel(model) && Array.isArray(event.payload.input)
				? event.payload.input.filter((item) => isJsonObject(item) && ["system", "developer"].includes(String(item.role)))
				: [];
			const payload: JsonObject = { ...event.payload, input: [...instructions, ...input] };
	...
	pi.on("turn_end", (_event, ctx) => {
		if (legacyCompaction || !supportsNativeCompaction(ctx.model)) return;
	...
	pi.on("session_compact", (event, ctx) => {
		...
		if (
			...
			|| event.reason === "manual"
			|| !event.fromExtension
			|| !supportsNativeCompaction(ctx.model)
