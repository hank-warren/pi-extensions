...
/** Only the Codex-backed families published on Responses by pi-cliproxyapi-provider. */
export function isCpaCodexModel(model: unknown, cpaProviders: readonly string[]): model is Model<"openai-responses"> {
	if (!isJsonObject(model)) return false;
	return typeof model.provider === "string" && cpaProviders.includes(model.provider)
		&& model.api === "openai-responses"
		&& typeof model.id === "string"
		&& /^(?:.*\/)?gpt-(?:5\.6|6(?:\.\d+)?)(?:-|$)/.test(model.id);
}

export function isNativeCompactionModel(
	model: unknown,
	cpaProviders: readonly string[],
): model is Model<"openai-codex-responses" | "openai-responses"> {
	return isOpenAICodexModel(model) || isCpaCodexModel(model, cpaProviders);
}

export function modelKey(model: Pick<Model<any>, "provider" | "api" | "id" | "baseUrl">): string {
	const key = `${model.provider}:${model.api}:${model.id}`;
	// Preserve upstream direct-Codex checkpoints. Proxy checkpoints also bind to
	// their endpoint: reconfiguring a provider must not send opaque history elsewhere.
	return model.api === "openai-responses" ? `${key}:${resolveCpaResponsesUrl(model.baseUrl)}` : key;
}
...
export function resolveCpaResponsesUrl(baseUrl: string): string {
	if (!baseUrl?.trim()) throw new Error("CPA compaction requires the model's base URL.");
	const url = new URL(baseUrl.trim());
	if (!["http:", "https:"].includes(url.protocol) || url.username || url.password || url.search || url.hash) {
		throw new Error("CPA compaction requires an HTTP(S) base URL without credentials, query, or fragment.");
	}
...
/** CPA owns upstream OAuth. Never decode a proxy API key as a ChatGPT JWT. */
export function buildCpaHeaders(params: {
	apiKey: string;
	headers?: Record<string, string | null>;
	sessionId: string;
}): Headers {
	const headers = new Headers(Object.entries(params.headers ?? {}).filter(
...
export function buildCodexHeaders(params: {
	...
	const headers = buildCpaHeaders(params);
