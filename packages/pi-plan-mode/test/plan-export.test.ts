...
/**
 * Exports read the durable plan file, so each case needs its own agent dir in
 * addition to the export cwd.
 */
async function withTempDirectory(run: (directory: string) => Promise<void>) {
	...
	const agentDirectory = await mkdtemp(join(tmpdir(), "pi-plan-export-agent-"));
...
/**
 * Writes a plan file for a session id and returns the state entry that points
 * at it, standing in for a completed planning workflow.
 */
...
test("ready plan export ends Plan mode without triggering a model turn", async () => {
	await withTempDirectory(async (directory) => {
		...
		planMode(mock.pi, { readSettings: async () => ({ kind: "loaded" as const, settings: {} }) });
		const context = createMockContext({ cwd: directory, hasUI: true });
		await mock.events.get("session_start")?.[0]?.({}, context.ctx);
		await mock.commands.get("plan")?.handler("start", context.ctx);
		// Thinking level is a session setting Plan mode never writes to.
		...
		for (const { command, expectedPath } of [
			...
			planMode(mock.pi, {
				readSettings: async () => ({
					...
					settings: { defaultPlanExportPath: "@plans/default.md" },
				}),
			});
			const context = createMockContext({ cwd: directory, hasUI: true });
			await mock.events.get("session_start")?.[0]?.({}, context.ctx);
...
test("configured export defaults support an active plan in print and JSON modes", async () => {
	for (const scenario of [
		{ mode: "print", status: "▶ plan · implementing" },
		{ mode: "json", status: "▶ plan · implementing" },
	] as const) {
		await withTempDirectory(async (directory) => {
			const { entry } = await storedPlanEntry(`export-${scenario.mode}`);
			const mock = createMockPi({ activeTools: ["read", "edit"] });
			planMode(mock.pi, {
				readSettings: async () => ({
					kind: "loaded" as const,
			...
			const context = createMockContext({
				...
				hasUI: false,
				sessionManager: {
					getSessionId: () => `export-${scenario.mode}`,
					getBranch: () => [entry],
					getEntries: () => [entry],
				},
			});
...
test("plan export fails observably without a plan or on an existing path in no-UI modes", async () => {
	await withTempDirectory(async (directory) => {
		...
		const { entry: storedEntry } = await storedPlanEntry("export-existing");
		...
		assert.equal(jsonContext.statuses.get("plan-mode"), "▶ plan · implementing");
	});
});

test("completion and management TUI menus preview and use the configured export default", async () => {
	for (const scenario of ["automatic-ready", "manual-ready", "active"] as const) {
		await withTempDirectory(async (directory) => {
			...
			assert.equal(
				...
				scenario === "active" ? "▶ plan · implementing" : undefined,
			);
