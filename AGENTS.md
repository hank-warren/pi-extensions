...
`changeset publish` skips any version already on npm, so re-runs are safe. The `workflow_dispatch` trigger re-drives a publish that failed on infrastructure rather than package content; it publishes whatever the tree says is ahead of npm, and only when started from `main` — from any other ref the publish job is skipped, so a release branch can never publish or tag an unmerged commit.
