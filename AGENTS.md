...
That edge is worth its cost because `pi-permission-selector` is a pure library: nothing but the shared component ships. The opposite case was `guardian-transport.ts` (~40 lines), which `pi-herdr-auto-title` deep-imported from `pi-auto-permissions` — that made a pane-titling extension pull the whole 125 kB permissions engine into `node_modules` and take a version bump every time the guardian released, so it was **duplicated byte-for-byte** into both packages instead, policed by `DUPLICATED_SOURCES` in `scripts/validate.py`. `pi-herdr-auto-title` has since been removed and the transport has one home again. `DUPLICATED_SOURCES` now checks the versioned `plan-contract.ts` shared by `pi-tasks` and `pi-plan-mode`; update one copy and copy it to the other. The rule it leaves behind still holds: a helper needed by a second **extension** package is duplicated and registered in `DUPLICATED_SOURCES` (edit one copy, copy it over the other, keep package-specific details behind a parameter), never imported across packages by relative path. If a third caller ever needs it, promote it to a library package next to `pi-permission-selector`.
...
- **`.github/workflows/publish.yml`** — two jobs. Its **`gate`** job is the only gate on a main commit: it runs `npm test` (which runs the typecheck itself — no workflow repeats it) and `npm run scan-secrets`, fails on any unapplied `.changeset/*.md`, and reports whether any package version in the tree is not yet on npm. **Keep those two steps in sync with `ci.yml`'s `gate` job**; removing one silently drops all coverage for main. Its **`publish`** job `needs: gate` and is skipped entirely unless something is releasable (or the run is a `workflow_dispatch`), so an ordinary code merge never touches npm — no credentials, no `contents: write`, no registry calls.
  ...
  The `publish` job's `if` reads the **tree** — one `npm view <name>@<version>` per package, taken by the `gate` job — not a `paths:` trigger filter. A path filter matches what a push touched, so a release whose publish failed or was superseded would leave its versions ahead of npm with nothing left to re-trigger them, and the next unrelated merge would skip the publish. Reading the tree keeps "the next run converges" true for every subsequent commit. Any `npm view` failure, not just E404, counts as unpublished: the cost is a publish job that re-asks npm and finds nothing to do, where failing the gate would take main's only test run down with it.
...
`main` uses classic branch protection (not a ruleset; `gh api repos/hank-warren/pi-extensions/branches/main/protection` shows it): required status check `gate (pinned pi)` (strict), pull request required with **required approving reviews = 0** and **require review from code owners = on**, dismiss stale reviews, linear history, conversation resolution, no force-push or deletion, admins not enforced.

The zero-and-code-owners pair is deliberate. `.github/CODEOWNERS` is `* @hank-warren`, and GitHub never counts an author as an approver of their own pull request, so with the count at 1 every one of Hank's pull requests was unmergeable without `gh pr merge --admin` — there is no second code owner to approve it. At 0, a pull request from Hank is mergeable on green CI with a plain `gh pr merge --squash`; a pull request from anyone else still needs Hank's approval because the code-owner rule covers every path. The one known gap (community discussion #14866): a non-owner pushing onto an open pull request that Hank authored does not re-trigger the code-owner requirement. Do not "fix" this by raising the count back to 1 — that reintroduces `--admin` on every merge and bypasses the required check along with the review.
