/**
 * The shared `ExtensionAPI` / `ExtensionContext` double.
 *
 * It records rather than simulates: a test asserts what an extension *asked* Pi
 * to do. It lives outside `packages/` because a package may not import a
 * sibling's sources — `scripts/validate.py` carves out exactly this directory
...
type MockPiApi = {
	...
	registerMarkdownTransformer(transformer: MockHandler): void;
...
export function createMockPi(
	...
	// Plan mode must never mutate the session's active tool set; these record any
	// attempt so tests can assert the count is zero.
	...
	const rawPi: MockPiApi = {
		...
		// Pi's API is overloaded: a whole Provider, or a name plus a config. An
		// extension registering an alias provider (pi-multi-login) uses the first
		// form, so recording only the second lost the provider's identity.
		registerProvider(nameOrProvider: unknown, providerConfig?: unknown) {
			const named = typeof nameOrProvider === "string";
			const name = named ? nameOrProvider : String((nameOrProvider as { id?: unknown })?.id);
			const config = named ? providerConfig : nameOrProvider;
...
/** A model as `modelRegistry` hands it out; `find` matches on these two fields. */
...
/**
 * The `ctx.modelRegistry` facade.
 *
 * `runtime.completeSimple` is the seam extension-issued background model calls
 * dispatch through (see `packages/pi-auto-permissions/guardian-transport.ts`),
 * so a test scripts the model's answers with a `completeSimple` override. The
 * default throws: a call nobody scripted is a test asserting nothing, and that
 * should be loud rather than silently green.
 *
 * `models` and `providers` overrides feed `find`/`getAvailable`/`getProvider`.
 */
function createMockModelRegistry(overrides: Record<string, unknown>) {
	const models = (overrides.models as MockModel[] | undefined) ?? [];
	const providers = (overrides.providers as Record<string, unknown> | undefined) ?? {};
	const completeSimple =
		(overrides.completeSimple as ((...args: unknown[]) => unknown) | undefined) ??
		(() => {
			throw new Error("createMockContext: a model call was made with no completeSimple scripted");
		});
	return {
...
export function createMockContext(overrides: Record<string, unknown> = {}) {
	...
	const ctx = {
		...
		modelRegistry: overrides.modelRegistry ?? createMockModelRegistry(overrides),
		...overrides,
	};
